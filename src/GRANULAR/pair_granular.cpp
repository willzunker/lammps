...
void PairGranular::init_style()
{
  ...
  */
  ...
  if (! fix_flag) {
    ...
    modify->add_fix(fmt::format("{} all property/atom d_Ro d_Vcaps d_Vgeo d_Velas d_eps_bar d_dRnumerator d_dRdenominator d_Acon0 d_Acon1 d_Atot d_Atot_sum d_ddelta_bar0 d_ddelta_bar1 d_psi d_psi_b ghost yes", id_fix));
    ...
    index_ddelta_bar0 = atom->find_custom("ddelta_bar0",tmp1,tmp2);     // change in mean surface displacement: ddelta_bar^{n} 
    index_ddelta_bar1 = atom->find_custom("ddelta_bar1",tmp1,tmp2);     // change in mean surface displacement: ddelta_bar^{n+1}
